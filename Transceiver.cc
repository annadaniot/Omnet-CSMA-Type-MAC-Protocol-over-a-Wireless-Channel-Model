        ...
        {       
            if(msg->getIdentifier() == currentSignalStart->getIdentifier())
            {
                ...
                {                    
                   ...
                   {
                      //extract out MAC packet and put in transmissionindication
...
{ 
   ...
   if(kind == TRANSMISSION_REQUEST) 
   {
      ...
      {
         //respond with message of type TransmissionConfirm to the MAC with the field status set to statusBusy  
         EV <<"Transmission Request received but in the transmit state, sending statusBusy" << endl;  
         
         TransmissionConfirm* msgTransmissionConfirm = new TransmissionConfirm();  
         msgTransmissionConfirm->setStatus(statusBusy);   
    ...
    else if(kind == TRANSMISSION_REQUEST_STATE_1)
    {
  ...
  //Calculates the current signal power (in dBm) observed on the channel. 
  //Then waits csTime parameter 
  ...
  //of the paramter csThreshDBm otherwise false. // https://stackoverflow.com/questions/4089726/how-do-i-cast-a-parent-class-as-thechild-class void Transceiver::handleCSRequest(CSRequest *msg){ 
   ...
   for(int i = 0;i<Nnodes; i++)
   {
    ...
    macResponse->setKind(CS_RESPONSE);  
   EV << "Sending CS_RESPONSE with BusyChannel: " << macResponse->getBusyChannel() << endl;   
   EV << "CS_RESPOSE will arrive at T= " << simTime() + csTime << endl;   
   //wait for a time corresponding to the csTime parameter     scheduleAt(simTime()+csTime, macResponse);   
   //    sendDelayed(macResponse, simTime() + csTime, "tx2MacOut"); 
